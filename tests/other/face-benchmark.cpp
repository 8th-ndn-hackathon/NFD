...
private:
  ...
  {
    face1->afterReceiveInterest.connect([face2] (const Interest& interest, const EndpointId& endpointId) {
      face2->sendInterest(interest, 0);
    });
    face1->afterReceiveData.connect([face2] (const Data& data, const EndpointId& endpointId) {
      face2->sendData(data, 0);
    });
    face1->afterReceiveNack.connect([face2] (const ndn::lp::Nack& nack, const EndpointId& endpointId) {
